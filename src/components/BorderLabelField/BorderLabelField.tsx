...
export function BorderLabelField({
  ...
  labelClassName = 'text-[var(--oczki-primary-100)]',
  inputClassName = 'text-[var(--oczki-primary-100)]',
  className,
  ...inputProps
  ...
  return (
    <div className="flex w-full flex-col isolate items-start">
      ...
      <input
        className={`z-[1] h-[46px] w-full border border-[var(--oczki-secondary-200)] bg-transparent px-3 pb-[13px] pt-3 outline-none focus:border-[var(--oczki-primary-300)] ${inputClassName} ${className ?? ''}`}
        id={id}
        {...inputProps}
      />
    </div>
